...
function TaskDetailsPage(props: { params: { taskId: string } }) {
	const { tasks, workflow, editTask, deleteTask } = useStorage()
	...
	const onSubmit = (values: NewTaskValues) => {
		if (currentTask) {
			const task: ITask = {
				id: props.params.taskId.toUpperCase(),
				title: values.title,
	...
	const handleDeleteTask = () => {
		try {
			deleteTask(props.params.taskId.toUpperCase())
			push("/")
			toast.info("Task has been deleted")
		} catch (error) {
			const err = error as string
			toast.error(`Task hasn't been deleted, ${err}`)
		}
	}
	...
	useEffect(() => {
		if (!currentTask) {
			const findTask = tasks.find(
				(f, i) => f.id?.toUpperCase() === props.params.taskId.toUpperCase()
			)
			if (findTask) {
				const jsTask = toJS(findTask)
				setCurrentTask(jsTask)
				reset({
					title: jsTask.title,
					htmlContent: jsTask.htmlContent,
					tags: jsTask.tags,
					asDraft: false,
				})
				setTaskField(jsTask.fields)
			} else {
				push("/")
			}
		}
	...
	useEffect(() => {
		const findTask = tasks.find((f, i) => f.id?.toUpperCase() === props.params.taskId.toUpperCase())
		if (findTask) {
			findTask.fields = taskField
			const jsTask = toJS(findTask)
			setCurrentTask(jsTask)
		}
	}, [taskField])
	...
	const getResolve = () => {
		const state = currentTask?.fields.find(f => f.fieldType === "state")
		if (state) {
			const _v = state.value as IFieldState
			return _v.states[_v.selectedIndex].isResolved
		}
		return false
	}
				...
				<div className="flex flex-col relative xs:mt-4 lg:mt-0">
					<div className="flex gap-4 items-center xs:flex-col xs:gap-1 xs:items-start sm:items-center sm:gap-4 sm:flex-row">
						<button
							...
							onClick={() => {
								navigator.clipboard.writeText(
									`${props.params.taskId.toUpperCase()} ${currentTask.title}`
								)
							}}
						>
							{currentTask.id}
						...
						<p className="text-neutral-400">
							Created {formatDistance(currentTask.createdAt, new Date(), { addSuffix: true })}
						</p>
						{currentTask.modifiedAt && (
							<p className="text-neutral-400">
								Modified {formatDistance(currentTask.modifiedAt, new Date(), { addSuffix: true })}
							</p>
						)}
					</div>
					{isEdit ? (
						<>
							...
							<br />
							<div className="flex gap-2 pb-[10rem] xs:pb-5 lg:pb-[10rem] xs:flex-col sm:flex-row">
						...
						<div className="pt-5 xs:pb-10">
							...
							<div
								...
								dangerouslySetInnerHTML={{ __html: currentTask.htmlContent }}
							></div>
						</div>
					)}
				</div>
				<div className="relative xs:row-start-2 xs:pb-[10rem] lg:row-start-1 lg:col-start-2">
...
const c = observer(TaskDetailsPage)
export { c as CTaskDetailsPage }
