...
function BackupPage(props: { searchParams: { [key: string]: string | null } }) {
	const {
		register,
		handleSubmit,
		setValue,
	...
	const { notifications, clearAll: clearAllNotifications } = notificationService
	...
	const onSubmit = (values: BackupValues) => {
		if (workflow) {
			const backupObject: TBackup = {
				...
				notifications: values.notifications ? toJS(notifications) : [],
			}
			...
			link.click()
			if (props.searchParams.transfer) {
				clearAll()
				clearStorage()
